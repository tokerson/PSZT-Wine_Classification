...
# print(training_output_set)

n_epoch = 200

for i in range(0, n_epoch):
    loss_sum = 0
    for j in range(0, len(training_input_set)):
        result = network.feed_forward(training_input_set[j])
        network.backward_propagation(training_output_set[j][0], result, training_input_set[j])
        # print("Epoch= %d, data_row=%f, error=%f, expected=%f" % (i, j, result, training_outputs[j][0]))
        loss_sum += abs(result - training_output_set[j][0])
...
correct = 0
print("\n===TESTING===\n")
for i in range(0, len(testing_input_set)):
    result = network.feed_forward(testing_input_set[i]) \
        # if round(result, 1) == testing_outputs[i][0]:
    if get_rating(result) == get_rating(testing_output_set[i][0]):
